...
-(void) drawMapRect:(MKMapRect)mapRect zoomScale:(MKZoomScale)zoomScale inContext:(CGContextRef)context{
    ...
    for (int i=0;i< polyline.pointCount;i++){
        ...
        if (i==0){
            ...
            //get bounding box based on current point and previous point,
            // if the current line we want doesn't inside the current map rect at all
            // we abandon drawing.
            CGRect validBox = CGRectMake((point.x > prevPoint.x?prevPoint.x:point.x),
                                         (point.y > prevPoint.y?prevPoint.y:point.y),
                                         abs(point.x - prevPoint.x),
                                         abs(point.y - prevPoint.y));
            if (!CGRectIntersectsRect(validBox, mapRectCG))
                continue;
