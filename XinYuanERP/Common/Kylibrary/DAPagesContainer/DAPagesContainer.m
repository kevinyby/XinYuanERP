...
- (void)viewDidLoad
{
    ...
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0.,
                                                                     self.topBarHeight+12,
                                                                     CGRectGetWidth(self.view.frame),
                                                                     CGRectGetHeight(self.view.frame) - self.topBarHeight-12)];
    ...
    self.topBar.font=  [UIFont fontWithName:@"Arial" size:[FrameTranslater convertFontSize:(18)]];
    self.topBar.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleWidth;
    self.topBar.delegate = self;
    
    self.topBar.tabItemGap = _topBarItemGap;
 
    self.topBar.tabItemOriginX = _topBarOriginX;
...
- (void)setSelectedIndex:(NSUInteger)selectedIndex animated:(BOOL)animated
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(itemAtIndex:didSelectInDAPagesContainer:)]) {
        [self.delegate itemAtIndex:_selectedIndex didSelectInDAPagesContainer:self];
    }
//    [self.delegate itemAtIndex:_selectedIndex didSelectInDAPagesContainer:self];
}
...
//        self.topBar.itemTitles = [viewControllers valueForKey:@"title"];
        [self.topBar setItemTitles:[viewControllers valueForKey:@"title"] withImageArray:btns];
...
- (void)itemAtIndex:(NSUInteger)index didSelectInPagesContainerTopBar:(DAPagesContainerTopBar *)bar
{
    [self setSelectedIndex:index animated:YES];
     if (self.delegate&&[self.delegate respondsToSelector:@selector(itemAtIndex:didSelectInDAPagesContainer:)]) {
          [self.delegate itemAtIndex:index didSelectInDAPagesContainer:self];
     }
