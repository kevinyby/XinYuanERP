...
-(id)initWithViewController:(UIViewController*)viewController
{
    ...
    if(self)
    {
        ...
        __weak FPPopoverController *self_ = self;
        [_touchView setTouchedOutsideBlock:^{
            [self_ dismissPopoverAnimated:YES]; 
        }];
        
        
//        self.contentSize = CGSizeMake(200, 300); //default size
        self.contentSize = CGSizeMake(150, 200); //default size
...
-(void)presentPopoverFromPoint:(CGPoint)fromPoint
{
    ...
    if(windows.count > 0)
    {
        self.window = nil;
        self.parentView = nil;
        self.window = [windows objectAtIndex:0];
...
//        _window = [[windows objectAtIndex:0] retain];
        ...
        {
            self.parentView = nil;
            self.parentView = [_window.subviews objectAtIndex:0];
...
//    [_fromView release]; _fromView = [fromView retain];
    [self presentPopoverFromPoint:[self originFromView:self.fromView]];
}
