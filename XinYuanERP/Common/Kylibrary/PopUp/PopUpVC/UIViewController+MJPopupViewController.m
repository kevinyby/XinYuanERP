...
#define kMJPopupDismissButton  @"kMJPopupDismissButton"
...
- (UIButton*)mj_popupDismissButton {
    return objc_getAssociatedObject(self, kMJPopupDismissButton);
}

- (void)setMj_popupDismissButton:(UIButton *)mj_mj_popupDismissButton {
    objc_setAssociatedObject(self, kMJPopupDismissButton, mj_mj_popupDismissButton, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)presentPopupView:(UIView*)popupView animationType:(MJPopupViewAnimation)animationType dismissed:(void(^)(void))dismissed
{
    ...
    UIScrollView *overlayView = [[UIScrollView alloc] initWithFrame:CGRectMake(100, 200, 400, 300)];
    overlayView.contentSize = CGSizeMake(popupView.frame.size.width, popupView.frame.size.height);
    ...
    self.mj_popupDismissButton = [UIButton buttonWithType:UIButtonTypeCustom];
    self.mj_popupDismissButton.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    self.mj_popupDismissButton.backgroundColor = [UIColor redColor];
    self.mj_popupDismissButton.frame = CGRectMake(overlayView.frame.origin.x - 10, overlayView.frame.origin.y - 10, 20, 20);
    
    popupView.alpha = 0.0f;
    [overlayView addSubview:popupView];
    [sourceView addSubview:overlayView];
    [sourceView addSubview:self.mj_popupDismissButton];
    
    [self.mj_popupDismissButton addTarget:self action:@selector(dismissPopupViewControllerWithanimation:) forControlEvents:UIControlEventTouchUpInside];
    switch (animationType) {
        case MJPopupViewAnimationSlideBottomTop:
        case MJPopupViewAnimationSlideBottomBottom:
        case MJPopupViewAnimationSlideTopTop:
...
//        self.mj_popupBackgroundView.alpha = 0.0f;
        ...
        [self.mj_popupDismissButton removeFromSuperview];
        self.mj_popupDismissButton = nil;
